...
function analyzeAsset(priceHistory, currentPrice, assetName) {
  ...
  const indicators = [
    ...
    { name: 'Stochastic RSI', value: stochRsi ? stochRsi.toFixed(1) : 'N/A', ...scores.stochRsi, weight: weights.stochRsi },
    { name: 'MA Crossover (7/25)', value: sma7 && sma25 ? `${fmtPrice(sma7)} / ${fmtPrice(sma25)}` : 'N/A', ...scores.maCrossover, weight: weights.maCrossover },
    ...
    { name: 'Williams %R (14)', value: williamsR ? williamsR.toFixed(1) : 'N/A', ...scores.williamsR, weight: weights.williamsR },
    { name: 'Volatilitas', value: volatility ? `${volatility.toFixed(2)}%` : 'N/A', ...scores.volatility, weight: weights.volatility },
    { name: 'Support / Resistance', value: sr.support ? `${fmtPrice(sr.support)} / ${fmtPrice(sr.resistance)}` : 'N/A', ...scores.supportResistance, weight: weights.supportResistance },
  ];
